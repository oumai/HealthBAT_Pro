                     ...
                     failure:(void (^)(NSError * error))failure
{
    ...
    [XMCenter sendRequest:^(XMRequest *request) {
        ...
        if (baseModel.ResultCode != 0) {
            //数据异常失败，走failure方法
            if (failure) {
                NSError * error = [[NSError alloc] initWithDomain:@"CONNECT_FAILURE" code:baseModel.ResultCode userInfo:@{NSLocalizedDescriptionKey:baseModel.ResultMessage,NSLocalizedFailureReasonErrorKey:@"未知",NSLocalizedRecoverySuggestionErrorKey:@"未知"}];
                failure(error);
            }
            return ;
        }
