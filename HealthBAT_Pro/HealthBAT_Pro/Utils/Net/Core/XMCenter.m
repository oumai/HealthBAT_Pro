...
- (void)xm_successWithResponse:(id)responseObject forRequest:(XMRequest *)request {
    ...
    if (self.consoleLog) {
        if (request.requestType == kXMRequestDownload) {
            NSLog(@"\n============ [XMResponse Data] ===========\nrequest download url: %@ \nrequest methodType: %@ \nresponse data: %@\n==========================================\n", request.url, [self xm_requestHttpMethodType:request.httpMethod],responseObject);
        } else {
            if (request.responseSerializerType == kXMResponseSerializerRAW) {
                NSLog(@"\n============ [XMResponse Data] ===========\nrequest url: %@ \nrequest methodType: %@ \nresponse data: \n%@\n==========================================\n", request.url, [self xm_requestHttpMethodType:request.httpMethod],[[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding]);
            } else {
