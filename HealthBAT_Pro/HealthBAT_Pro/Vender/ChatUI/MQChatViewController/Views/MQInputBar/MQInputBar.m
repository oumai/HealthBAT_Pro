...
@property (nonatomic, weak) BATMoreView *moreView;
...
resignKeyboardHighlightedImage:(UIImage *)resignKeyboardHighlightedImage
{
    if (self = [super init]) {
        ...
        moreBtn = [[BATMoreButton alloc] init];
        [moreBtn setImage:[UIImage imageNamed:@"moreBtn"] forState:UIControlStateNormal];
        [moreBtn setImage:[UIImage imageNamed:@"moreBtn"] forState:UIControlStateHighlighted];
        [moreBtn addTarget:self action:@selector(cameraClick) forControlEvents:UIControlEventTouchUpInside];
        moreBtn.frame      = CGRectMake(self.frame.size.width - senderImageWidth - kMQInputBarHorizontalSpacing, (self.frame.size.height - senderImageWidth)/2, senderImageWidth, senderImageWidth);
...
resignKeyboardHighlightedImage:(UIImage *)resignKeyboardHighlightedImage
{
    if (self = [super init]) {
        ...
        moreBtn = [[BATMoreButton alloc] init];
        [moreBtn setImage:[UIImage imageNamed:@"moreBtn"] forState:UIControlStateNormal];
        [moreBtn setImage:[UIImage imageNamed:@"icon-zk"] forState:UIControlStateSelected];
        [moreBtn addTarget:self action:@selector(moreClick) forControlEvents:UIControlEventTouchUpInside];
        moreBtn.frame      = CGRectMake(self.frame.size.width - senderImageWidth - kMQInputBarHorizontalSpacing, (self.frame.size.height - senderImageWidth)/2, senderImageWidth, senderImageWidth);
        moreBtn.inputView = moreView;
...
resignKeyboardHighlightedImage:(UIImage *)resignKeyboardHighlightedImage
{
    if (self = [super init]) {
        ...
        self.chatTableView      = tableView;
        originalChatViewFrame   = tableView.frame;
        enableSearchDisease     = isSearchDisease;
        enableSendImage         = enableImage;
        enableSendVoice         = enableVoice;
        photoSenderImage        = photoImage;
        photoSenderHighlightedImage = photoHighlightedImage;
        ...
        [cameraBtn addTarget:self action:@selector(cameraClick) forControlEvents:UIControlEventTouchUpInside];
//        cameraBtn.frame      = enableSendImage ? CGRectMake(kMQInputBarHorizontalSpacing, (self.frame.size.height - senderImageWidth)/2, senderImageWidth, senderImageWidth) : CGRectMake(0, 0, 0, 0);
        cameraBtn.frame      = CGRectMake(0, 0, 0, 0);

        moreBtn = [[BATMoreButton alloc] init];
        moreBtn.layer.cornerRadius = 5.0f;
        [moreBtn setTitle:@"发送" forState:UIControlStateNormal];
        [moreBtn setBackgroundColor:BASE_COLOR];
        [moreBtn addTarget:self action:@selector(sendText:) forControlEvents:UIControlEventTouchUpInside];
        moreBtn.frame      = CGRectMake(self.frame.size.width - senderImageWidth*2 - kMQInputBarHorizontalSpacing, (self.frame.size.height - senderImageWidth)/2, senderImageWidth*2, senderImageWidth);
...
//    microphoneBtn.frame = CGRectMake(self.frame.size.width - senderImageWidth - kMQInputBarHorizontalSpacing, (self.frame.size.height - senderImageWidth)/2, senderImageWidth, senderImageWidth);
    ...
    microphoneBtn.frame = cameraBtn.frame;
    ...
    if (enableSearchDisease) {
        recordBtn.frame = CGRectMake(cameraBtn.frame.origin.x + cameraBtn.frame.size.width + kMQInputBarHorizontalSpacing, (originalFrame.size.height - textViewHeight)/2, originalFrame.size.width - cameraBtn.frame.origin.x - senderImageWidth*2 - kMQInputBarHorizontalSpacing * 3 , textViewHeight);
...
-(void)cameraClick
{
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:[MQBundleUtil localizedStringForKey:@"cancel"] style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        
    }];
    
    UIAlertAction *galleryAction = [UIAlertAction actionWithTitle:[MQBundleUtil localizedStringForKey:@"select_gallery"] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if (self.delegate) {
    ...
    UIAlertAction *cameraAction = [UIAlertAction actionWithTitle:[MQBundleUtil localizedStringForKey:@"select_camera"] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if (self.delegate) {
            if ([self.delegate respondsToSelector:@selector(sendImageWithSourceType:)]) {
...
- (void)moreClick
{
    ...
    if (moreBtn.isFirstResponder) {
        [moreBtn resignFirstResponder];
    } else {
        [moreBtn becomeFirstResponder];
    }
}
...
-(void)inputKeyboardWillShow:(NSNotification *)notification
{
    ...
    } else if (moreBtn.isFirstResponder) {
        float keyboardHeight;
...
//    microphoneBtn.frame = enableSendVoice ? CGRectMake(self.frame.size.width - senderImageWidth - kMQInputBarHorizontalSpacing, self.frame.size.height-senderImageWidth-kMQInputBarVerticalSpacing, senderImageWidth, senderImageWidth) : CGRectMake(0, 0, 0, 0);
    ...
    microphoneBtn.frame = enableSendVoice ? cameraBtn.frame : CGRectMake(0, 0, 0, 0);
}
