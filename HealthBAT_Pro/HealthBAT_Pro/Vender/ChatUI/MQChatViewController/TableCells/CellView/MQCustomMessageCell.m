...
@interface MQCustomMessageCell() <TTTAttributedLabelDelegate, UIActionSheetDelegate, UIAlertViewDelegate>

@end

@implementation MQCustomMessageCell {
...
- (void)updateCellWithCellModel:(id<MQCellModelProtocol>)model {
    if (![model isKindOfClass:[MQCustomCellModel class]]) {
        NSAssert(NO, @"传给MQTextMessageCell的Model类型不正确");
        return ;
    }
    MQCustomCellModel *cellModel = (MQCustomCellModel *)model;
    ...
    if (cellModel.attributeRanges.count > 0) {
        for (NSValue *rangeValue in cellModel.attributeRanges) {
            NSRange range = [rangeValue rangeValue];
            [textLabel addLinkToTransitInformation:@{@"value" : [cellModel.cellText attributedSubstringFromRange:range].string} withRange:range];
...
- (void)attributedLabel:(TTTAttributedLabel *)label didSelectLinkWithTransitInformation:(NSDictionary *)components {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"SEARCH_DISEASE_SEND_MESSAGE" object:nil userInfo:components];
