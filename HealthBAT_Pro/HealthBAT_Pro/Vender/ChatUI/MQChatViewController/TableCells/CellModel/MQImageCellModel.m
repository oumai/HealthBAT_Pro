...
- (MQImageCellModel *)initCellModelWithMessage:(MQImageMessage *)message
    ...
    if (self = [super init]) {
        ...
        [self setModelsWithContentImage:nil cellFromType:message.fromType cellWidth:cellWidth];
        ...
        //原图路径
        self.orginPath = message.orginPath;
        if (!message.image) {
...
//                [self setModelsWithContentImage:[MQChatViewConfig sharedConfig].incomingBubbleImage cellFromType:message.fromType cellWidth:cellWidth];
                ...
                [tempImageView sd_setImageWithURL:[NSURL URLWithString:message.imagePath] placeholderImage:[UIImage imageNamed:@"icon_pictures_default"] options:SDWebImageProgressiveDownload completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
//- (void)setupCellFromType:(MQChatMessageFromType)cellFromType cellWidth:(CGFloat)cellWidth
//{
//    
//    //限定图片的最大直径
//    CGFloat maxBubbleDiameter = ceil(cellWidth / 2);  //限定图片的最大直径
//    CGSize contentImageSize = CGSizeMake(20, 20);
//    
//    //先限定图片宽度来计算高度
