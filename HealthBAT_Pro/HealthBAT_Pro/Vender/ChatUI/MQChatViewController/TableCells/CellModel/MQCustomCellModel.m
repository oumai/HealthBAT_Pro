...
@implementation MQCustomCellModel

- (MQCustomCellModel *)initCellModelWithMessage:(MQCustomMessage *)message
                                     ...
                                     delegate:(id<MQCellModelDelegate>)delegator
{
    if (self = [super init]) {
        ...
        //文字高度
        CGFloat messageTextHeight = [MQStringSizeUtil getHeightForAttributedText:self.cellText textWidth:maxLabelWidth] + 7;
        //判断文字中是否有emoji
        ...
        self.attributeRanges = [NSMutableArray array];
        NSArray *separationArray = [message.content componentsSeparatedByString:@"\n"];
        for (NSString *string in separationArray) {
            if ([string containsString:@"-"]) {
                NSRange tmpRange = [message.content rangeOfString:string];
                NSRange range;
                range.location = tmpRange.location+1;
                range.length = tmpRange.length-1;
...
- (MQChatBaseCell *)getCellWithReuseIdentifier:(NSString *)cellReuseIdentifer {
    return [[MQCustomMessageCell  alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellReuseIdentifer];
}
...
- (void)updateCellFrameWithCellWidth:(CGFloat)cellWidth {
    ...
    CGFloat maxLabelWidth = cellWidth - kMQCellAvatarToHorizontalEdgeSpacing - kMQCellAvatarDiameter - kMQCellAvatarToBubbleSpacing - kMQCellBubbleToTextHorizontalLargerSpacing - kMQCellBubbleToTextHorizontalSmallerSpacing - kMQCellBubbleMaxWidthToEdgeSpacing;
    //文字高度
    CGFloat messageTextHeight = [MQStringSizeUtil getHeightForAttributedText:self.cellText textWidth:maxLabelWidth] + 1;
    //判断文字中是否有emoji
    if ([MQChatEmojize stringContainsEmoji:[self.cellText string]]) {
        NSAttributedString *oneLineText = [[NSAttributedString alloc] initWithString:@"haha" attributes:self.cellTextAttributes];
        CGFloat oneLineTextHeight = [MQStringSizeUtil getHeightForAttributedText:oneLineText textWidth:maxLabelWidth];
