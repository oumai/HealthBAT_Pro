...
@implementation MQFaceCellModel
...
//        [message.content setYy_attributes:self.cellTextAttributes];
        [message.content setYy_lineSpacing:kMQTextCellLineSpacing];
        [message.content setYy_lineHeightMultiple:1.0];
        [message.content setYy_lineBreakMode:NSLineBreakByWordWrapping];
        [message.content setYy_alignment:NSTextAlignmentLeft];
        [message.content setYy_font:[UIFont systemFontOfSize:kMQCellTextFontSize]];
        
        if (message.fromType == MQChatMessageOutgoing) {
            
            [message.content setYy_color:[MQChatViewConfig sharedConfig].outgoingMsgTextColor];

        } else {
            
            [message.content setYy_color:[MQChatViewConfig sharedConfig].incomingMsgTextColor];
        }
...
//        CGFloat messageTextHeight = [MQStringSizeUtil getHeightForAttributedText:self.cellText textWidth:maxLabelWidth];
        ...
        CGSize size = CGSizeMake(maxLabelWidth, CGFLOAT_MAX);
        YYTextLayout *layout = [YYTextLayout layoutWithContainerSize:size text:self.cellText];
        CGFloat messageTextHeight = layout.textBoundingSize.height;
        CGFloat messageTextWidth = layout.textBoundingSize.width;
...
- (MQChatBaseCell *)getCellWithReuseIdentifier:(NSString *)cellReuseIdentifer {
    return [[MQFaceMessageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellReuseIdentifer];
}
