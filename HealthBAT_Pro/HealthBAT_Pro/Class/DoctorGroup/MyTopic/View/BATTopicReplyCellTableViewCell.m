...
//        [comment yy_setColor:UIColorFromHEX(0x0182eb, 1) range:NSMakeRange(0, model.UserName.length)];
    ...
    if (model.ReplyUserName.length > 0) {
        comment = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@回复%@：%@",model.UserName,model.ReplyUserName,model.ReplyContent]];
        [comment setYy_font:[UIFont systemFontOfSize:12]];
        comment.yy_color = UIColorFromHEX(0x666666, 1);
        [comment yy_setColor:UIColorFromHEX(0x0182eb, 1) range:NSMakeRange(0, model.UserName.length)];
        [comment yy_setColor:UIColorFromHEX(0x0182eb, 1) range:NSMakeRange(model.UserName.length + 2, model.ReplyUserName.length)];
    } else {
        comment = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：%@",model.UserName,model.ReplyContent]];
        [comment setYy_font:[UIFont systemFontOfSize:12]];
        comment.yy_color = UIColorFromHEX(0x666666, 1);
