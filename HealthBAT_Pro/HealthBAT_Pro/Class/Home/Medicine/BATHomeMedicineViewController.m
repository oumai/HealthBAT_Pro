...
@interface BATHomeMedicineViewController ()<UIWebViewDelegate>
...
@implementation BATHomeMedicineViewController
...
//    [NSURLProtocol registerClass:[CustomURLProtocol class]];
    ...
    [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[self medicineUrl]]]];
...
-(void)viewWillDisappear:(BOOL)animated {
    ...
    //[TalkingData trackPageEnd:@"国医馆"];
    
    [BATUserPortrayTools saveOperateModuleRequestWithURL:@"/kmStatistical-sync/saveOperateModule" pathName:@"首页-医药馆" moduleId:1 beginTime:self.beginTime];
}
...
- (NSString *)medicineUrl {
    ...
    if (!LOGIN_STATION) {
        ...
        //未登录
        strURL = [NSString stringWithFormat:@"%@/kmHealthMall-web/explosion.jsp?deviceCode=%@",MALL_URL,[self getPostUUID]];
    }
    else {

        //已经登录
        strURL = [NSString stringWithFormat:@"%@/kmHealthMall-web/explosion.jsp?userName=%@&deviceSource=1&userId=%ld&deviceCode=%@",MALL_URL,person.Data.PhoneNumber,(long)login.Data.ID,[self getPostUUID]];
    }
