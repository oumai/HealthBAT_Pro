...
@end
@implementation BATSymptomNameCell
...
-(void)configCellWithModel:(BATSymptomModel *)model{
    ...
    self.model = model;
    self.nameLb.text = self.model.Data.SYMPTOM_NAME;
    self.detailLb.text = self.model.Data.BRIEFINTRO_CONTENT;
...
+(CGFloat)HeightWithModel:(BATSymptomModel *)model {
    ...
    BATSymptomNameCell *cell = [[BATSymptomNameCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"symptonNameCell"];
...
{
//    //开关置反
//    self.model.isNameOpen = !self.model.isNameOpen;
//    
    //刷新表格
    if (self.nameBlock) {
        self.nameBlock(self.path);
    }
    [self animationStart];
}
