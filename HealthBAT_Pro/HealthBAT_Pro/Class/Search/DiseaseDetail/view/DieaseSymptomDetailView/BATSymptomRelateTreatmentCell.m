 //
//  BATSymptomRelateTreatmentCell.m
//  HealthBAT_Pro
//
...
@interface BATSymptomRelateTreatmentCell ()
...
@end
@implementation BATSymptomRelateTreatmentCell
...
-(void)configCellWithModel:(BATSymptomModel *)model{
    ...
    self.examineDetailLb.text = self.model.Data.RELATED_INSPECTIONS_NLIST;
    self.treatmeDetailntLb.text = self.model.Data.IDENTIFICATION_DETAIL;
    self.nursedDetailLb.text = self.model.Data.PREVENTION_DETAIL;
    
    if (model.Data.isExaimBtnShow) {
        self.examineBtn.hidden = NO;
    }else {
        self.examineBtn.hidden = YES;
    }
    
    if (model.Data.isTreatmentBtnShow) {
        self.treatmentBtn.hidden = NO;
    }else {
        self.treatmentBtn.hidden = YES;
    }
    
    if (model.Data.isNurseBtnShow) {
...
+(CGFloat)HeightWithModel:(BATSymptomModel *)model {
    BATSymptomRelateTreatmentCell *cell = [[BATSymptomRelateTreatmentCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"symptonRelateCell"];
...
-(UIImageView *)iconView{
    if (!_iconView) {
        _iconView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"icon07"]];
    }
    return _iconView;
}
...
-(UIView *)examineDocView{
    if (!_examineDocView) {
        ...
        _examineDocView.backgroundColor = UIColorFromHEX(0X81ab54, 1);
    }
...
-(UIView *)treatmentDocView{
    if (!_treatmentDocView) {
        ...
        _treatmentDocView.backgroundColor = UIColorFromHEX(0X81ab54, 1);
    }
...
-(UIView *)nursedDocView{
    if (!_nursedDocView) {
        ...
        _nursedDocView.backgroundColor = UIColorFromHEX(0X81ab54, 1);
    }
