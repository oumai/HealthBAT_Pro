...
{
    UICollectionViewLayoutAttributes *layoutAttributes = self.itemAttributes.lastObject;
    NSLog(@"%@",NSStringFromCGSize(layoutAttributes.size));
    NSLog(@"%@",NSStringFromCGRect(layoutAttributes.frame));
    
    CGFloat heigth = 0;

    if (self.itemAttributes.count <=2) {
       heigth = layoutAttributes.frame.size.height + layoutAttributes.frame.origin.y+15+25;
    }else {
       heigth = layoutAttributes.frame.size.height + layoutAttributes.frame.origin.y+43;
    }
    
    [[NSNotificationCenter defaultCenter] postNotificationName:@"SENDHEIGHT" object:@(heigth)];
