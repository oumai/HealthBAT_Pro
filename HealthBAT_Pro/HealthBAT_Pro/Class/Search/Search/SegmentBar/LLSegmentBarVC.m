...
- (void)viewWillLayoutSubviews{
    ...
    if (self.segmentBar.superview == self.view) {
        ...
        self.segmentBar.frame = CGRectMake(0, 60, self.contentView.width, 45);
...
- (void)segmentBar:(LLSegmentBar *)segmentBar didSelectIndex:(NSInteger)toIndex fromIndex:(NSInteger)fromIndex{
    ...
    if ([self.delegate respondsToSelector:@selector(LLSegmentBarVC:didSelectIndex:fromIndex:)]) {
        [self.delegate LLSegmentBarVC:segmentBar didSelectIndex:toIndex fromIndex:fromIndex];
    }
