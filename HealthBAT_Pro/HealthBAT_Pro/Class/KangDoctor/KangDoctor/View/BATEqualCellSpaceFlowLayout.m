...
@interface BATEqualCellSpaceFlowLayout(){
    //在居中对齐的时候需要知道这行所有cell的宽度总和
    CGFloat _sumCellWidth ;
}
@end

@implementation BATEqualCellSpaceFlowLayout
-(instancetype)init{
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    UICollectionViewLayoutAttributes *lastLayoutAttributes = layoutAttributes.lastObject;
    
    if (self.resetCollectionRect && CGRectGetMaxY(lastLayoutAttributes.frame) > 0) {
        self.resetCollectionRect(CGRectGetMaxY(lastLayoutAttributes.frame)+5);
    }
